cmd-showaccessreaders-desc = Переключает отображение разрешений читателя доступа на карте.
...
cmd-showaccessreaders-status = Установите наложение отладки считывателя доступа на { $status }.
